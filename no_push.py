...
df=pd.read_excel('configurations.xlsx', skiprows=[1,])
...
for i, row in df.iterrows():
    ...
    mainPath, 'my_files', str(row["weather"])))
    ...
    Office = Zone(window_area=row["window_area"],
                ...
                heating_supply_system=eval(row["heating_supply_system"]),
                cooling_supply_system=eval(row["cooling_supply_system"]),
                heating_emission_system=eval(row["heating_emission_system"]),
                cooling_emission_system=eval(row["cooling_emission_system"]),)
